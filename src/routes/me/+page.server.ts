...
export const load: PageServerLoad = async ({ locals, url, platform }) => {
	...
	const [pendingIntros, digestCount, matchedRoles] = await Promise.all([
	...
	return { pendingIntros: pendingIntros?.n ?? 0, digestCount: digestCount?.n ?? 0, matchedRoles: matchedRoles?.n ?? 0 };
};
